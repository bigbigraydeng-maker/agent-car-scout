...
class TradeMeScraper {
  constructor() {
    this.baseUrl = 'https://www.trademe.co.nz';
    this.selectors = {
  ...
  async scrape(model, limit = 50) {
    console.log(`🔍 开始抓取 TradeMe - ${model}...`);
    ...
    try {
      ...
      // 导航到搜索页面
      const searchUrl = `${this.baseUrl}/a/motors/cars/search?year_min=2005&price_max=8000&odometer_max=160000&listing_type=private&q=${encodeURIComponent(model)}`;
      ...
      const listings = await page.evaluate((selectors, model) => {
        ...
        cards.forEach(card => {
          try {
            ...
            console.log('Error parsing TradeMe listing:', e);
          }
        });
      ...
      console.log(`✅ TradeMe 抓取完成，获取 ${limitedListings.length} 条数据`);
...
module.exports = new TradeMeScraper();